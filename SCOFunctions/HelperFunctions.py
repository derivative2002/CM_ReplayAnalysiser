...
def get_account_dir(path: Optional[str] = None) -> str:
    ...
    if os.path.isdir(user_folder):
        ...
        # One drive location (通过环境变量)
        if "ONEDRIVE" in os.environ:
            account_path = os.path.abspath(os.path.join(os.environ["ONEDRIVE"], 'Documents', 'StarCraft II', 'Accounts'))
            if os.path.isdir(account_path):
                return account_path
                
        # 直接检查OneDrive路径 (针对中文Windows)
        onedrive_path = os.path.abspath(os.path.join(user_folder, 'OneDrive', 'Documents', 'StarCraft II', 'Accounts'))
        if os.path.isdir(onedrive_path):
            return onedrive_path
            
        # 直接检查OneDrive路径 (针对英文Windows)
        onedrive_path = os.path.abspath(os.path.join(user_folder, 'OneDrive'))
        if os.path.isdir(onedrive_path):
            account_path = os.path.abspath(os.path.join(onedrive_path, 'Documents', 'StarCraft II', 'Accounts'))
            if os.path.isdir(account_path):
                return account_path
